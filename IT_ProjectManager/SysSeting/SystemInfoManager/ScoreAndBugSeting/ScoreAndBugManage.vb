Public Class ScoreAndBugManage
...
#Region "屬性"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Dim IsSystemSeting As Boolean = False
    ''' <summary>
    ''' 評分管理是否為編輯狀態
    ''' treu 為是
    ''' false為否
    ''' </summary>
    ''' <remarks></remarks>
    Dim GradeStandardEdit As Boolean = False
    ''' <summary>
    ''' 系統名稱管理是否為編輯狀態
    ''' true 為是
    ''' false為否
    ''' </summary>
    ''' <remarks></remarks>
    Dim SystemManagerEdit As Boolean = False
    ''' <summary>
    ''' bug類型管理是否為編輯狀態
    ''' true 為是
    ''' false為否
...
#Region "載入數據"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Sub loadGradeStandardData()
        Try

            Dim sql As String = Nothing

            sql = "select * from IT_GradeStandard where GS_PID='1'"
            ...
            If dt.Rows.Count > 0 Then
                gridGradeStandard.DataSource = dt
            Else
                gridGradeStandard.DataSource = Nothing
            End If
    ...
    Sub loadAccessData()
        Try
            ...
            sql = "select * from IT_GradeStandard where GS_PID='3'"
...
#Region "評分管理"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Sub SaveGradeStandardData()

        Dim sql As String = Nothing

        sql = "insert into  IT_GradeStandard values('1','1','" & txtName.Text.Trim() & "','" & txtRemarks.Text.Trim & "')"

        Try

            If ExecuteNonQuery(sql) <> 1 Then
                MessageBox.Show("保存數據失敗？", "項目管理系統", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
...
#Region "為GRIDVIEW繪製顏色"
    ...
    Private Sub GridView1_CustomDrawCell(sender As System.Object, e As DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs) Handles GridView2.CustomDrawCell, GridView3.CustomDrawCell, GridView1.CustomDrawCell
    ...
    Sub CustomDrawCell(ByVal e As DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs)
        ...
        If Convert.ToInt32(tempInt) Mod 2 = 0 Then
...
#Region "管理Grid雙擊"
    ...
    Private Sub xtControl_SelectedPageChanged(sender As System.Object, e As DevExpress.XtraTab.TabPageChangedEventArgs) Handles xtControl.SelectedPageChanged
        ...
        GradeStandardEdit = False
        SystemManagerEdit = False
        BugManagerEdit = False

    End Sub

    '' 雙擊查看或修改評分項目
    Private Sub GridControl1_DoubleClick(sender As System.Object, e As System.EventArgs) Handles gridGradeStandard.DoubleClick

        Claerlbl()

        TempAutoID = GridView1.GetFocusedRowCellValue("GS_ID")
        TempName = GridView1.GetFocusedRowCellValue("GS_Name")
        TempRemarks = GridView1.GetFocusedRowCellValue("GS_Remark")

        SetText()

        GradeStandardEdit = True
    ...
    Private Sub btSave_Click(sender As System.Object, e As System.EventArgs) Handles btSave.Click
        ...
        Select Case CInt(xtControl.SelectedTabPageIndex)
            ...
            Case 0  '' 評分名稱管理模式
                ...
                If GradeStandardEdit Then
                    EditGradeStandardData(True, "1")
                Else
                    EditGradeStandardData(False, "1")
                End If
            ...
            Case 2 ''考核信息
                ...
                If AssessEdit Then ''修改考核類型
                    EditGradeStandardData(True, "3")
                Else '' 保存考核類型管理
                    EditGradeStandardData(False, "3")
                End If
...
#Region "刪除數據"
    ...
    '' 執行Dle按鍵
    Private Sub GridControl1_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles gridGradeStandard.KeyDown

        If e.KeyCode = Keys.Delete Then

            TempAutoID = GridView1.GetFocusedRowCellValue("GS_ID")
            TempName = GridView1.GetFocusedRowCellValue("GS_Name")
            If DelData(TempName, TempAutoID, "1") Then
                '' 移除指定行
    ...
    Function DelData(ByVal GS_Name As String, ByVal GS_ID As Integer, ByVal AssessID As Integer) As Boolean
        ...
        Try
            ...
            If GS_Name <> "" Or GS_ID < 0 Then
                ...
                sql = "delete IT_GradeStandard where GS_ID='" & GS_ID & "' and GS_PID='" & AssessID & "' and GS_Name='" & GS_Name & "' "
